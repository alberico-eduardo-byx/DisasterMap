    ...
    for event in events:
        ...
        if category_data:
            category, created = Category.objects.get_or_create(
                id=category_data["id"], defaults={"name": category_data["title"]}
            )
        else:
            category = None
        ...
        event_obj, created = Event.objects.update_or_create(
            ...
            defaults={
                ...
                "geometry": event.get(
                    "geometry", []
                ),  # Armazena a lista completa de geometrias
                "sources": event.get(
                    "sources", []
                ),  # Armazena a lista completa de fontes
                "country": None,
            },
        )
...
def update_event_countries_view(request):
    ...
    for event in events:
        try:
            ...
            geometry_data = (
                event.geometry[0] if isinstance(event.geometry, list) else None
            )
            if geometry_data:
                ...
                if coordinates and len(coordinates) == 2:
                    latitude, longitude = (
                        coordinates[1],
                        coordinates[0],
                    )  # Inverter ordem para latitude/longitude
                    # Geocodifica o local para obter o país
                    location = geolocator.reverse(
                        (latitude, longitude), language="en", timeout=5
                    )
...
def get_events_by_id(request, event_id):
    ...
    serialized_events_filtered_by_id = EventSerializer(
        events_filtered_by_id, many=True
    ).data
